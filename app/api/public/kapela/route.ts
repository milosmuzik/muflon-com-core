...
// používá ho web radiomuflon.com a appka muflon-com-stats k obohacení
...
export async function GET(req: NextRequest) {
  ...
  const [pribehyRaw, udalostiRaw, sestava] = await Promise.all([
    najdiVerejnePribehy(interpret.id),
    najdiVerejneUdalosti(interpret.nazev),
    najdiSestavu(interpret.id),
  ]);

  const historie = interpret.historie ?? "";
  const pribehy = pribehyRaw;
